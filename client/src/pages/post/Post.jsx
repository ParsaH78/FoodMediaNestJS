...
export default function Post({user}) {
    ...
    const getRating = () => {
        ...
        rating.forEach((rate) => {
            for (let key in rate) {
              if (key === "user" && rate[key] === user._id) {
                return parseFloat(rate);
              }
            }
          })
        return {user: user._id, rate: 0}
    }


    const handleStars = async (e) => {
        setRate(e.target.value);
        const newRate = {user: user._id, rate: e.target.value};
    ...
    const delPost = async () => {
        dispatch(deletePost(data._id, navigate, user._id));
    }
    ...
    const addFavorite = async() => {
        try {
            await api.addToFavorite(data._id);
          } catch (error) {
            console.log("Error in Add To Favorite : ",  error.response);
          }
                        ...
                        <div className="postCommentBox">
                            ...
                            <div className="postCommentItemsBox">
                                <div className="postCommentItemUser">
                                    <div className="postCommentCreator">
                                        ...
                                        : PF + "person/noAvatar.png"} 
                                        alt="profile pic" />
                                        <span className="postCommentCreatorName">{user?.username}</span>
                                    </div>
                                </div>
                            ...
                            <div className="postCommentItemsBox">
                                ...
                                <div className="commentBox">
                                    ...
                                    {(comment.username === user.username || creator.username === user.username) &&
